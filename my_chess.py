...
class Chess(object):
    ...
    def display_analysis(self):
        wins = [self.analysis_info['result']['1-0'], self.analysis_info['result']['0-1'], self.analysis_info['result']['1/2-1/2'],]
        wins_sum = sum(wins)
        wins_percent = [100*wins[0]/ wins_sum, 100*wins[1]/ wins_sum, 100*wins[2]/ wins_sum]

        next_move = {key: value for key, value in self.analysis_info['next_move'].items() if True}
        top_moves = sorted(next_move.items(), key=lambda x: x[1], reverse=True)[:5]
        top_moves = [f"    {item[0]}: {100 * item[1]/wins_sum:.2f}% ({item[1]})" for item in top_moves]
        lines = [
            f"winning chance:",
            f"    white: {wins_percent[0]:.2f}% ({wins[0]})",
            f"    black: {wins_percent[1]:.2f}% ({wins[1]})",
            f"    draw: {wins_percent[2]:.2f}% ({wins[2]})"
            "  ",
            "popular next moves:",
        ] + top_moves
        ...
        line_surfaces = [small_font.render(line, True, white_color) for line in lines]

        # Blit the lines to the screen at the desired positions
        y_offset = 690  # Starting y-coordinate
        line_spacing = 30  # Spacing between lines
        for i, line_surface in enumerate(line_surfaces):
            self.screen.blit(line_surface, (50, y_offset + i * line_spacing))

        # todo

    def update_analysis_info(self):
        self.analysis_info = get_info(self.piece_location)            
